class NodeSchema:
    ...
    def run(self, input):
        ...
        self.prompt = self.generate_system_prompt(self.input_pydantic_model is not None, node_prompt=self.node_prompt,node_input = self.input.model_dump_json() if self.input_pydantic_model is not None else None)
...
class EdgeSchema:
    def __init__(self, from_node_schema, to_node_schema, state_condition_fn, new_input_from_state_fn):
...
class TakeOrderState(BaseModel):
    order: Order
    has_finished_ordering: bool = Field(description="whether the customer has finished ordering. This can only be true after you have explicitly confirmed with them that they have finished ordering, by asking questions like 'Anything else?'.")
...
class ConfirmOrderState(BaseModel):
    has_confirmed_order: bool = Field(description="whether the customer has confirmed their order")
