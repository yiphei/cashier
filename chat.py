...
def is_on_topic(MM, current_node_schema, all_node_schemas):
    ...
    if not is_on_topic:
        ...
        prompt = (
            ...
            " and a set of tools (i.e. functions), but all agents share the same background and guidelines. "
            "An AI agent can handle a message (whether user's or the agent's (i.e. 'role' = 'assistant')) if it is "
            "a case strictly covered by its expectation OR tools, meaning that the case is either mentioned in the expectation or can be successfully addressed using the available tools. "
            "Given the prior conversation and a list of AI agents,"
            " determine which agent can best handle the last message. "
            "Respond by returning the AI agent ID.\n\n"
            "# BACKGROUND\n\n"
            f"{BACKGROUND}\n\n"
            "# GUIDELINES\n\n"
            "<!--- This section enumerates important guidelines on how the agent should behave. These must be strictly followed --->\n"
            "## Response\n\n"
